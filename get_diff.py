...
while(True):
    ...
    ret, live = cap.read()
    time_reading = datetime.now(timezone.utc)
    timediff = time_reading - start
    #cv2.imshow('live', live)
    #proc.stdin.write(live.tostring())


    if start - last_mask_run > MASK_SPEED:

        # compare images
        # https://www.pyimagesearch.com/2017/06/19/image-difference-with-opencv-and-python/
        (score, diff) = structural_similarity(live, base, full=True, multichannel=True)
        ...
        timediff = now - start
        reading = time_reading - start
        ssim = ssim_time - time_reading
        rest = now - ssim_time
        print(
            f"Processing took {timediff.total_seconds()*1000:.0f} ms, "
            f"reading {reading.total_seconds()*1000:.0f}, "
